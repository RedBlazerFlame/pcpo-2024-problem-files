...
const ll max_n = 100'000ll;
const ll max_queries = 100'000ll;
const ll max_m = 100'000ll;
...
int main(int argc, char** argv) {
    ...
    ll Q = inf.readInt(1, max_queries, "Q");
    inf.readEoln();
    
    set<pair<ll, ll>> queries_set;
    while(Q--) {
        ll cnana = inf.readInt(1, N, "C_nana_i");
        inf.readSpace();
        ll clili = inf.readInt(1, N, "C_lili_i");
        inf.readEoln();

        ensuref(queries_set.count({cnana, clili}) == 0, "All queries must be distinct");
        queries_set.insert({cnana, clili});
    }
